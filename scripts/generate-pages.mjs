...
// ── the gate ──────────────────────────────────────────────────────────────
// The specification line repeats the card title where the names coincide; then only the rest stays.
const specLine = (p, l) => p.spec.name[l].startsWith(p.name[l] + ', ') ? p.spec.name[l].slice(p.name[l].length + 2) : p.spec.name[l]
...
${diagram(l)}
<div class="parts">
${parts.parts.map((p) => `<a class="part" id="${p.id}" href="${p.url}"><span class="w w-${p.spec.world}">${esc(p.spec.abbr)}</span><span class="part__text"><span class="part__title">${esc(p.name[l])}</span><span class="part__claim">${esc(p.sentence[l])}</span></span><span class="part__meta">${esc(specLine(p, l))} · ${esc(p.for[l])}</span><span class="part__link">${esc(p.url.replace(/^https?:\/\//, ''))} ${ARROW}</span></a>`).join('\n')}
</div>
