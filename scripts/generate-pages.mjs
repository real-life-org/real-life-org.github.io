...
// On the page the picture is written into the document so it follows the page's tokens and the
// colour-scheme toggle; its own style block goes, its variables map onto the page's. A click opens
// the standalone file, which follows the system scheme.
